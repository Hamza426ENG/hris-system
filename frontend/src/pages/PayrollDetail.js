...
import { ArrowLeft, Play, CheckCircle, XCircle, Download, Edit } from 'lucide-react';
...
export default function PayrollDetail() {
  ...
  const load = () => {
    setLoading(true);
    payrollAPI.get(id).then(r => setData(r.data)).catch(() => navigate('/payroll')).finally(() => setLoading(false));
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 no-print">
        <button onClick={() => navigate('/payroll')} className="flex items-center gap-2 text-oe-muted hover:text-oe-text transition-colors text-sm">
