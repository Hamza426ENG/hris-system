...
import { Plus, Eye, Play, CheckCircle, XCircle, Wallet } from 'lucide-react';
...
export default function Payroll() {
  ...
  // Quick create for current month
  const quickCreate = () => {
    const now = new Date();
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id} className="table-row cursor-pointer" onClick={() => navigate(`/payroll/${r.id}`)}>
                  ...
                  <td className="table-cell">
                    <div className="flex items-center gap-1">
                      <button onClick={e => { e.stopPropagation(); navigate(`/payroll/${r.id}`); }} className="p-1.5 hover:bg-oe-surface rounded text-oe-muted hover:text-oe-text transition-colors" title="View">
                        <Eye size={14} />
                      </button>
                      {r.status === 'processing' && (
